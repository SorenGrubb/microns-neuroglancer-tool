...
- **Nearest-cell identification** — finds the closest annotated nucleus and shows its MICrONS cell-type prediction (broad class + fine m-type, where available), estimated cortical layer, distance to the glia limitans, distance to the white matter border, nearest traced blood vessel (type + distance), and its three nearest neighbouring cells.
- **One-click Neuroglancer links** — the identified cell's name is itself the Neuroglancer link; an inline arrow button next to the coordinate fields builds the link and starts identification. Falls back to the wider minnie35/Img35 imagery and segmentation for coordinates outside the minnie65-predicted volume.
...
- **Guided decision-tree identification** — a step-by-step ultrastructural decision tree covering 23 leaf cell types (immune cells, vascular cells, neurons, glia, and leptomeningeal cell types), each with sourced criteria and a reference-image gallery. Each result screen can also pull up a random verified or MICrONS-predicted example of that same cell type for comparison.
- **Filter & bulk export** — filter the whole dataset by cell type(s), vessel proximity (type + distance), glia-limitans distance, white-matter-border distance, cortical layer, and/or a coordinate bounding box (with a live top/side-view preview against the dataset extents); preview the match count, download every match as an Excel file (coordinates, root/nucleus IDs, m-type, vessel info, 3 nearest neighbours), or open every match at once in a single zoomed-out 3D Neuroglancer view (segmentation + nuclei only, no EM).
...
All the core data (nucleus positions, cell-type predictions, m-types, the verified-identification dataset, traced vessel outlines, and traced pia/white-matter surfaces) is packed directly into the HTML file as base64-encoded binary arrays, so it works fully offline except for the actual Neuroglancer viewer, Google sign-in, and the reference-image thumbnails (loaded from the companion `.js` file).
