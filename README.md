...
Built by **Søren Grubb** to make identifying brain cortical cells in electron microscopy easier and more collaborative. Built with the assistance of Claude AI.

## What it does

The tool is organized into four tabs: **Jump** (coordinate lookup and cell identification), **Filter and show** (dataset-wide filtering and bulk export), **References**, and **Statistics dashboard** (live/precomputed charts built from the dataset and the community-reporting backend).

### Jump tab

- **Coordinate lookup** — paste a coordinate (voxels, nanometres, or micrometres) and jump straight to it; paste `x, y, z` into the first field and it splits automatically. The last 3 searched coordinates are kept as one-click history chips, and a collapsible panel lets you search directly by root ID or nucleus ID instead. A "Recently viewed cells" panel and Back/Forward navigation let you step between cells you've already looked at without re-searching.
- **Nearest-cell identification** — finds the closest annotated nucleus and shows its MICrONS cell-type prediction (broad class + fine m-type, where available), estimated cortical layer, distance to the glia limitans, distance to the white matter border, nearest traced blood vessel (type + distance), and its three nearest neighbouring cells. Every coordinate and ID shown is click-to-copy, and a "compute volume" action can fetch and measure the cell's mesh on demand.
- **One-click Neuroglancer links** — the identified cell's name is itself the Neuroglancer link; an inline arrow button next to the coordinate fields builds the link and starts identification. Falls back to the wider minnie35/Img35 imagery and segmentation for coordinates outside the minnie65-predicted volume. Every generated 3D view also draws the cortical layer boundaries, pia (glia limitans) surface and white-matter surface as labelled 3D line annotations, and automatically zooms out further for neurons than for smaller cell types, since a full dendritic/axonal reconstruction can span far more than a soma. The tissue is mounted at an angle in this dataset, and the pia and white matter are not flat — measured against a best-fit plane they carry roughly 178 µm of relief. The layer boundaries therefore follow the traced surfaces point by point along X rather than being drawn as a single tilted plane, so they sit on the tissue across the whole width instead of only where you clicked. Each band holds one depth through the Z thickness of the volume. Where a view includes the wider minnie35/Img35 volume the bands extend across it too; the surfaces were only traced over part of that width, so segments beyond the tracing are drawn dimmed to mark them as extrapolated rather than measured. The viewer opens at Neuroglancer's native orientation, so the EM appears as it is stored and the layer boxes lie on it at the tissue's own angle.
...
- **Community identification & reporting** — lets users browse a random unclassified cell (or a random example of a given type) and identify it through the guided tree, with a 1–5 certainty rating and optional Google sign-in for attribution and points. Depending on the situation, users can: flag a disagreement with MICrONS' own prediction, confirm they independently agree with it (more agreement builds more confidence in a call), contribute a first identification for a cell MICrONS never predicted, log a brand-new cell MICrONS' detector missed entirely (with duplicate-report detection), flag a nucleus detection as a segmentation artefact, split a single detection that's actually two or more fused nuclei, or report the location of an organelle or extracellular structure (61 kinds across 13 groups, including centrioles and primary cilia) on an already-identified cell. All of this is shared back with the MICrONS consortium and shown to other users browsing the same cell. A per-cell classification history shows every past submission, and signed-in users can correct or restore their own prior submissions (admins can restore anyone's).
- **Guided decision-tree identification** — a step-by-step ultrastructural decision tree covering 16 broad cell-type categories (immune cells, vascular cells, neurons, glia, and leptomeningeal cell types), expanding to 37 total leaf-level identifications once neuron, leptomeningeal and pericyte subtypes are included, each with sourced criteria and a reference-image gallery. Pericytes can optionally be resolved into ensheathing, mesh and thin-strand subtypes from 3D shape and position along the vessel, using criteria taken from volume-EM measurements rather than light microscopy alone (see References); a plain "Pericyte" option is always offered, since these grade into one another rather than forming discrete classes. A fast type-ahead search jumps straight to any leaf type from the main screen or mid-tree. Each result screen can also pull up a random verified or MICrONS-predicted example of that same cell type for comparison.
- **Cell contacts** — an on-demand, mesh-proximity search (not synapse-based) for physical membrane contact between the current cell and other cell types within a chosen radius and touch-gap threshold, with a colour-coded 3D Neuroglancer view of every contact point found and an Excel export of the coordinates.
- **Astrocyte–synapse proximity** *(experimental)* — a structural proxy for tripartite synapses: finds astrocyte mesh processes near the current cell's synapses (from the CAVE connectivity data), using the same mesh-proximity method as Cell contacts. It flags distance only and does not confirm the process actually ensheathes the synapse.
- **Connectivity panel** — queries the CAVE synapse table for the current cell's incoming/outgoing synapse partners, with a per-partner-type breakdown; each query is saved and feeds the live Connectivity charts in the Statistics dashboard.
- **Google sign-in** — optional account for attribution, a per-user points/level system, submission history, and a downloadable "my reports" export of everything you've contributed.

### Filter and show tab

Filter the whole dataset by cell type(s), vessel proximity (type + distance), glia-limitans distance, white-matter-border distance, cortical layer, Allen Institute V1-column membership, one or more coordinate bounding boxes (with a live top/side-view preview against the dataset extents), and/or "needs attention" tiers (unclassified, verified-but-unpredicted, community-reported, or totally untouched); unclassified cells can optionally be included. Preview the match count, step through matches one at a time with Previous/Next (resumable), download every match as an Excel file (coordinates, root/nucleus IDs, m-type, vessel info, 3 nearest neighbours, community-report tally, classification history), or open every match at once in a single colour-coded 3D Neuroglancer view (optionally including the traced vasculature outline, raw EM imagery, and the V1-column wireframe).

### References tab

The sourced reference list and image gallery backing the decision tree (see [References](#references) below).

### Statistics dashboard tab

An embedded, live-updating dashboard mixing precomputed and on-demand data from the community-reporting backend: nucleus volume as a share of whole-cell mesh volume, primary-cilium length and nucleus-to-centriole distance by cell type, mesh-contact touch-point counts by cell-type pair, and connectivity (synapse partner) graphs built from every recorded sample.

### Cross-cutting

Dark/light theme toggle, and a layout that works on both desktop and mobile.
...
- Manually traced pia (glia limitans) and white-matter surfaces, used to estimate cortical depth and layer. Both are shifted 32 µm upwards from the raw tracing before use: compared against the EM they sat systematically low, and the same offset was visible in the cell positions — 2.2% of nuclei fell *above* the traced pia, which is anatomically impossible, dropping to 0.17% after the correction (a residue consistent with genuine leptomeningeal cells, which do sit above the glia limitans). The two surfaces are shifted by the same amount, so cortical thickness and the proportional layer boundaries derived from it are unchanged. The cause of the offset is not yet established — most likely a difference in origin between the tracing and EM coordinate frames — so it is kept as a single named constant rather than baked into the packed data. Layer boundaries are taken from Ledderose et al. (2023) and rescaled proportionally to each location's own locally traced cortical thickness, since both that source and MICrONS are chemically fixed tissue and fixation is known to shrink cortex — see [References](#references).
