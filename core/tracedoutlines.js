...
/* A viewer that reads polylines, preferring one the page already offers him, so a link that has
...
async function buildTracedOrganelleLayers(ids,want,say,opts){
  ...
  Object.keys(byKind).sort().forEach(function(k){
    ...
    byKind[k].forEach(function(g,gi){
      ...
      /* opts.base names the viewer the annotations are FOR, which is what decides their shape:
