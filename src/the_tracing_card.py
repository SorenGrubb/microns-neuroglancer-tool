...
── AND A THIRD PASS, AN HOUR LATER ───────────────────────────────────────────────────────────
  ...
  AND A FIFTH: HOW BIG IT IS. *"We need to calculate the organelle volumes also and add the volumes
    to the data for the cell when submitting."* core/traceloft.js gained volume() -- Cavalieri's
    estimator over the outlined areas, with the even-odd rule so a contour inside another is a hole
    -- and this card puts the number on the pad WHILE DRAWING (a volume you only see after
    submitting cannot tell you that you traced one section too few), beside the button, and into the
    submission. The number stored is the number he saw: measured once, in tracingCurrent(), and
    carried from there by the kept object, the Blender export and the POST alike.
    backend/src_a_tracing_says_how_big_it_is.py keeps it on the index row.

  AND A SIXTH: A PEN DRAWS IT. *"Could we have an option to click and draw the mouse around a
    structure to mimic using a pen to draw, so that you can use a e.g. Kamvas 13 pad or a Apple pen
    to draw the polylines?"* A tick box, remembered per browser, and switched on by itself the first
    time a stylus touches the pad (pointerType "pen") -- announced, not silent. The work is in
    core/tracepad.js: a raw stroke is hundreds of samples a fraction of a pixel apart, and it is
    thinned by distance while drawing and by Ramer-Douglas-Peucker on lifting, so what lands is an
    ORDINARY contour that every existing correction gesture works on. Grabbing a vertex still beats
    starting a stroke, and shift still pans -- only a plain press on empty canvas changed meaning.

  AND A SEVENTH: SEVERAL OF THE SAME THING. *"I want the option to draw more than one organelle of
    the same type, the extra added organelles should have different colors, so you can distinguish
    them, and when publishing they should have different numbers, also when shown on the cell
    identity page."* A cell has forty mitochondria. A contour now carries which structure it belongs
    to (core/tracepad.js), the strip under the canvas shows them in the export's own palette, and
    ONE press adds them as separate tracings -- separate rows, separate files, separate volumes --
    numbered on from what the cell already carries rather than restarted at one. The last clause is
    src/traced_structures_on_the_cell_panel.py: the numbers are worth having only where the things
    they distinguish are listed together.

  AND AN EIGHTH: EACH ONE ITS OWN THING. *"I would like an option to identify the organelles
    identities individually, so standard is that they are all the same, but if you click a button
    you can identify them individually."* A tick box beside the type, offered only once there are
    two to tell apart. Off -- the default, and the case that happens -- one type for everything on
    the pad. On, the type box belongs to the structure SELECTED in the Drawing strip, so one pass
    over a cell can log a mitochondrion, a lysosome and two vesicles. Turning it on SEEDS every
    structure with the type already chosen, so the act of turning it on never changes what anything
    is; and the numbering is per series, so two mitochondria and a lysosome are Mitochondrion 1,
    Mitochondrion 2 and Lysosome 1.
...
async function pad3DDraw(){
  ...
  try {
    ...
    geos.forEach(function(q, i){
      ...
      siblings.push({ geo: q, tint: padInstTint(lofts[i].inst), alpha: 1 });
    });
    ...
    UJ.mesh3d.show(host, geo, { lead: lead, ghosts: drawn, view: PAD3D_VIEW,
                                tint: padInstTint(lofts[subject].inst),
